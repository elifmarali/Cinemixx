...
function FilmList({ list, type }: { list: Array<IMultipleList | ISingleList>; type: string }) {
  return (
          ...
          if ("List" in filmItem) {
                ...
                <div className="flex flex-row gap-4">
                  {filmItem.List.map((item) => (
                    <Link href={`/movies/${item.id}`} key={item.id}>
                      <Image
                        src={typeof item.file === "string" ? item.file : item.file ? URL.createObjectURL(item.file) : `https://image.tmdb.org/t/p/original${item.poster_path}`}
                        alt={item.title}
            ...
            return (
              <Link href={`/movies/${filmItem.id}`} key={filmItem.id}>
                <Image
                  src={typeof filmItem.file === "string" ? filmItem.file : filmItem.file ? URL.createObjectURL(filmItem.file) : `https://image.tmdb.org/t/p/original${filmItem.poster_path}`}
                  alt={filmItem.title}
                  width={180}
                  height={270}
                  loading="lazy"
